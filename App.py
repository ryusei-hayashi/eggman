...
if st.button(f'Search {"EgGMAn" if y.size else "Random"}', type='primary'):
    try:
        ...
        st.dataframe(q.iloc[numpy.argsort(((numpy.stack(q['vec']) - z) ** 2).sum(1))[:99], :5], hide_index=True, column_config={'URL': st.column_config.LinkColumn(), 'Time': st.column_config.TimeColumn(format='mm:ss')})
    except:
